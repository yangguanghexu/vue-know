<template>
  <div>
    <div class="loading" v-if="articleData == null">
      <van-loading type="spinner" color="rgb(25, 137, 250)" />
    </div>
    <div class="article">
      <div v-if="articleData" class="article-details">
        <div class="article-headline">
          <div class="article-title">{{articleData.title}}</div>
          <div class="article-source">{{articleData.image_source}}</div>
          <img :src="articleData.image" alt />
        </div>
        <div class="article-main" v-html="articleData.body"></div>
      </div>
      <div v-if="this.extraData" class="moreList">
        <div class="backBtn" @click="()=> this.$router.go(-1)">
          <van-icon name="arrow-left" />
        </div>
        <div class="commentBtn" @click="toComment">
          <van-icon name="comment-o" :info="this.extraData.comments" />
        </div>
        <div class="praiseBtn">
